...
from alphabeter.apis.vision import VisionHttp
from alphabeter.helpers import gmaps, playgames, auth, calendar as gcalendar, ia
from alphabeter.helpers.knowledge import get_user_type_definition
...
async def hunt(as_client: httpx.AsyncClient, email_address: str, json_file: Path=None):
    ...
    alphabeter_creds = await auth.load_and_auth(as_client)

    #gb.rc.print("[+] Target found !", style="sea_green3")

    people_pa = PeoplePaHttp(alphabeter_creds)
    # vision_api = VisionHttp(alphabeter_creds)
    ...
    player_results = await playgames.search_player(alphabeter_creds, as_client, email_address)
    if player_results:
        ...
        _, player = await playgames.get_player(alphabeter_creds, as_client, player_candidate.id)
    ...
    cal_found, calendar, calendar_events = await gcalendar.fetch_all(alphabeter_creds, as_client, email_address)
    ...
    if json_file:
        import json
        from alphabeter.objects.encoders import AlphabeterEncoder;
        with open(json_file, "w", encoding="utf-8") as f:
            f.write(json.dumps(json_results, cls=AlphabeterEncoder, indent=4))
