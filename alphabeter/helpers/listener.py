...
from alphabeter.objects.base import SmartObj
...
class Server(BaseHTTPRequestHandler):    
    ...
    def do_GET(self):
        if self.path == "/alphabeter_ping":
            self._set_response()
            self.wfile.write(b"alphabeter_pong")

    def do_POST(self):
        if self.path == "/alphabeter_feed":
            content_length = int(self.headers['Content-Length']) # <--- Gets the size of data
            post_data = self.rfile.read(content_length) # <--- Gets the data itself
            self.data_bridge.data = post_data.decode('utf-8')

            self._set_response()
            self.wfile.write(b"alphabeter_received_ok")
...
def run(server_class=HTTPServer, handler_class=Server, port=60067):
    ...
    try:
        print(f"Alphabeter is listening on port {port}...")
