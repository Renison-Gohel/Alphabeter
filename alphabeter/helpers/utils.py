...
from alphabeter import version as current_version
from alphabeter.lib.httpx import AsyncClient


def get_httpx_client() -> httpx.AsyncClient:
    """
        Returns a customized to better support the needs of Alphabeter CLI users.
    """
    return AsyncClient(http2=True, timeout=30)
...
def show_version():
    ...
    print()
    gb.rc.print(f"> Alphabeter {current_version.metadata.get('version', '')} ({current_version.metadata.get('name', '')}) <".center(53), style="bold")
    print()
    if new_version:
        gb.rc.print(f"🥳 New version {new_metadata.get('version', '')} ({new_metadata.get('name', '')}) is available !", style="bold red")
        gb.rc.print(f"🤗 Run 'pipx upgrade alphabeter' to update.", style="bold light_pink3")
    else:
...
def check_new_version() -> tuple[bool, dict[str, str]]:
    """
        Checks if there is a new version of Alphabeter available.
    """
    req = httpx.get("https://raw.githubusercontent.com/RC/Alphabeter/master/alphabeter/version.py")
