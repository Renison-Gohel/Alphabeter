...
from alphabeter.errors import *
from alphabeter.helpers.utils import *
from alphabeter.helpers import listener
from alphabeter.helpers.knowledge import get_domain_of_service, get_package_sig
from alphabeter.knowledge.services import services_baseurls
from alphabeter.helpers.auth import *
...
async def android_master_auth(as_client: httpx.AsyncClient, oauth_token: str) -> Tuple[str, List[str], str, str]:
    ...
    for keyword in ["Token", "Email", "services", "firstName", "lastName"]:
        if keyword not in resp:
            raise AlphabeterAndroidMasterAuthError(f'Expected "{keyword}" in the response of the Android Master Authentication.\nThe oauth_token may be expired.')
...
async def android_oauth_app(as_client: httpx.AsyncClient, master_token: str,
    ...
    for keyword in ["Expiry", "grantedScopes", "Auth"]:
        if keyword not in resp:
            raise AlphabeterAndroidAppOAuth2Error(f'Expected "{keyword}" in the response of the Android App OAuth2 Authentication.\nThe master token may be revoked.')
...
async def gen_osid(as_client: httpx.AsyncClient, cookies: Dict[str, str], generated_osids: dict[str, str], service: str) -> None:
    ...
    if not "OSID" in req.cookies:
        raise AlphabeterOSIDAuthError("[-] No OSID header detected, exiting...")
...
async def check_master_token(as_client: httpx.AsyncClient, master_token: str) -> str:
    ...
    except AlphabeterAndroidAppOAuth2Error:
        return False
    return True

async def gen_cookies_and_osids(as_client: httpx.AsyncClient, alphabeter_creds: AlphabeterCreds, osids: list[str]=[*services_baseurls.keys()]):
    from alphabeter.apis.accounts import Accounts
    accounts_api = Accounts(alphabeter_creds)
    is_logged_in, uber_auth = await accounts_api.OAuthLogin(as_client)
    if not is_logged_in:
        raise AlphabeterLoginError("[-] Not logged in.")
    ...
    alphabeter_creds.cookies = cookies
    osids = await gen_osids(as_client, cookies, osids)
    alphabeter_creds.osids = osids

async def check_and_gen(as_client: httpx.AsyncClient, alphabeter_creds: AlphabeterCreds):
    """Checks the validity of the cookies and generate new ones if needed."""
    if not await check_cookies(as_client, alphabeter_creds.cookies):
        await gen_cookies_and_osids(as_client, alphabeter_creds)
        if not await check_cookies(as_client, alphabeter_creds.cookies):
            raise AlphabeterLoginError("[-] Can't generate cookies after multiple retries. Exiting...")

    alphabeter_creds.save_creds(silent=True)
...
def auth_dialog() -> Tuple[Dict[str, str], str] :
    ...
    """
    choices = ("You can facilitate configuring Alphabeter by using the Alphabeter Companion extension on Firefox, Chrome, Edge and Opera here :\n"
                "=> https://github.com/RC/alphabeter_companion\n\n"
                "[1] (Companion) Put Alphabeter on listening mode (currently not compatible with docker)\n"
...
async def load_and_auth(as_client: httpx.AsyncClient, help=True) -> AlphabeterCreds:
    """Returns an authenticated AlphabeterCreds object."""
    creds = AlphabeterCreds()
    try:
        creds.load_creds()
    except AlphabeterInvalidSession as e:
        if help:
            raise AlphabeterInvalidSession(f"Please generate a new session by doing => alphabeter login") from e
        else:
            raise e
