...
class AlphabeterCreds(SmartObj):
    """
        This object stores all the needed credentials that Alphabeter uses,
    ...
    def __init__(self, creds_path: str = "") -> None:
        ...
        if not creds_path:
            cwd_path = Path().home()
            alphabeter_folder = cwd_path / ".shadows/alphabeter"
            if not alphabeter_folder.is_dir():
                alphabeter_folder.mkdir(parents=True, exist_ok=True)
            creds_path = alphabeter_folder / "creds.m"
    ...
    def load_creds(self, silent=False) -> None:
        ...
        if Path(self.creds_path).is_file():
            try:
                ...
                raise AlphabeterInvalidSession("Stored session is corrupted.")
        else:
            raise AlphabeterInvalidSession("No stored session found.")
        
        if not self.are_creds_loaded():
            raise AlphabeterInvalidSession("Stored session is incomplete.")
        if not silent:
