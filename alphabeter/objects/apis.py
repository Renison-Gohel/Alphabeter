from alphabeter.errors import AlphabeterCorruptedHeadersError
from alphabeter.helpers.knowledge import get_origin_of_key, get_api_key
...
class GAPI(SmartObj):
    ...
    def _load_api(self, creds: AlphabeterCreds, headers: Dict[str, str]):
        if not creds.are_creds_loaded():
            raise AlphabeterInsufficientCreds(f"This API requires a loaded AlphabeterCreds object, but it is not.")

        if not is_headers_syntax_good(headers):
            raise AlphabeterCorruptedHeadersError(f"The provided headers when loading the endpoint seems corrupted, please check it : {headers}")
    ...
    def _load_endpoint(self, endpoint: EndpointConfig):
        ...
        if endpoint.authentication_mode in ["sapisidhash", "cookies_only"]:
            if not (cookies := self.creds.cookies):
                raise AlphabeterInsufficientCreds(f"This endpoint requires the cookies in the AlphabeterCreds object, but they aren't loaded.")

        if (key_name := endpoint.require_key):
            if not (api_key := get_api_key(key_name)):
                raise AlphabeterInsufficientCreds(f"This endpoint requires the {key_name} API key in the AlphabeterCreds object, but it isn't loaded.")
        ...
        if endpoint.authentication_mode == "sapisidhash":
            if not (sapisidhash := cookies.get("SAPISID")):
                raise AlphabeterInsufficientCreds(f"This endpoint requires the SAPISID cookie in the AlphabeterCreds object, but it isn't loaded.")
    ...
    async def _check_and_gen_authorization_token(self, as_client: httpx.AsyncClient, creds: AlphabeterCreds):
    ...
    async def _query(self, endpoint_name: str, as_client: httpx.AsyncClient, base_url: str, params: Dict[str, Any]={}, data: Any=None) -> httpx.Response:
        ...
        if endpoint.verb == "GET":
            ...
            if endpoint.data_type == "data":
                ...
                raise AlphabeterUnknownRequestDataTypeError(f"The provided data type {endpoint.data_type} wasn't recognized by Alphabeter.")
        else:
            raise AlphabeterUnknownVerbError(f"The provided verb {endpoint.verb} wasn't recognized by Alphabeter.")
