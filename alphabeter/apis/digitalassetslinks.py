...
import alphabeter.globals as gb
from alphabeter.objects.apis import GAPI, EndpointConfig
from alphabeter.parsers.digitalassetslinks import DalStatements
...
class DigitalAssetsLinksHttp(GAPI):
    def __init__(self, creds: AlphabeterCreds, headers: Dict[str, str] = {}):
        super().__init__()
    ...
    async def list_statements(self, as_client: httpx.AsyncClient, website: str="",
        ...
        if website and (android_package_name or android_cert_fingerprint):
            raise AlphabeterParamsInputError(f"[DigitalAssetsLinks API list statements] website and {android_package_name if android_package_name else android_cert_fingerprint} can't be both put at the same time.")
        elif not website and not (android_package_name and android_cert_fingerprint):
            raise AlphabeterParamsInputError("[DigitalAssetsLinks API list statements] Please , android_package_name and android_cert_ingerprint.")
        elif not (website or android_package_name or android_cert_fingerprint):
            raise AlphabeterParamsInputError("[DigitalAssetsLinks API list statements] Please choose at least one parameter between website, android_package_name and android_cert_ingerprint.")
