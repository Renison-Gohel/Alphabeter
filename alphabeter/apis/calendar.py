...
from alphabeter.parsers.calendar import Calendar, CalendarEvents
...
class CalendarHttp(GAPI):
    ...
    async def get_events(self, as_client: httpx.AsyncClient, calendar_id: str, params_template="next_events",
        ...
        if not params_templates.get(params_template):
            raise AlphabeterParamsTemplateError(f"The asked template {params_template} for the endpoint {endpoint.name} wasn't recognized by Alphabeter.")
