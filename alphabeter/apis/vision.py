...
from alphabeter.parsers.vision import VisionFaceDetection
...
class VisionHttp(GAPI):
    ...
    async def detect_faces(self, as_client: httpx.AsyncClient, image_url: str = "", image_content: str = "",
        ...
        if not data_templates.get(data_template):
            raise AlphabeterParamsTemplateError(f"The asked template {data_template} for the endpoint {endpoint.name} wasn't recognized by Alphabeter.")

        # Inputs checks
        if image_url and image_content:
            raise AlphabeterParamsInputError("[Vision API faces detection] image_url and image_content can't be both put at the same time.")
        elif not image_url and not image_content:
            raise AlphabeterParamsInputError("[Vision API faces detection] Please choose at least one parameter between image_url and image_content.")
