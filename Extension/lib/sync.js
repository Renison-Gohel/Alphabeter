...
async function main(choice) {
    ...
    var storage = await browser.storage.local.get("alphabeter");
    if ("alphabeter" in storage && storage.alphabeter.ready) {
        var encoded_cookies = utils.encodeCookies(storage.alphabeter.cookies);
        var finished = false;

        if (choice == "server") {
            status_label.innerHTML = "Feeding Alphabeter with cookies...";
            let success = await utils.sendCookies(encoded_cookies);
            if (success) {
                finished = true;
                status_label.innerHTML = "Finished !";
            } else {
                status_label.innerHTML = "Can't contact Alphabeter on 127.0.0.1:60067 ...";
            };
        ...
        if (finished) {
            await browser.storage.local.remove("alphabeter");
